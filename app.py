...
def validar_datos(form):
    """Valida que los 8 campos existan, sean numéricos, no sean negativos y estén
    dentro del rango físicamente razonable definido en CAMPOS (min/max).
    ...
    for campo in CAMPOS:
        ...
        if valor < campo["min"] or valor > campo["max"]:
            errores.append(
                f"El campo '{campo['etiqueta']}' debe estar entre {campo['min']} y {campo['max']} {campo['unidad']}."
            )
            continue
